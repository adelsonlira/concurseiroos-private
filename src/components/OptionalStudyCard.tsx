...
import { findCompetitionRuntimeDefinition, getDefaultCompetitionRuntimeDefinition } from "../config/concursos/registry";
...
export default function OptionalStudyCard({ localDate, context }: { localDate: string; context: OptionalStudyContext }) {
  ...
  const privateMaterialCatalog = (findCompetitionRuntimeDefinition(configuracao.concursoAlvoId) ?? getDefaultCompetitionRuntimeDefinition()).privateStudyMaterials;
  const automaticManualMatch = manualDiscipline && manualTopic ? findOptionalStudyMaterial(biblioteca, manualDiscipline.id, manualTopic.id, manualSubtopic?.id, privateMaterialCatalog) : { confidence: "none" as const };
  const manualMaterials = biblioteca.filter((item) => !item.isDeleted && (item.id === automaticManualMatch.material?.id || item.disciplinaId === manualDisciplineId || item.assuntoId === manualTopicId));
  const manualMatch = manualDiscipline && manualTopic && manualMaterialId
    ? findOptionalStudyMaterialById(biblioteca, manualMaterialId, manualDiscipline.id, manualTopic.id, manualSubtopic?.id, privateMaterialCatalog)
    : automaticManualMatch;
  const manualMaterial = manualMatch.material;
  ...
  const manualBaseOption: OptionalStudyRecommendationOption | undefined = manualDiscipline && manualTopic ? {
    ...selected,
  ...
  const manualOption = manualBaseOption
    ? validateOptionalStudyExecutionOption(manualBaseOption, "manual_optional", manualMatch, privateMaterialCatalog)
    : undefined;
  const displayedOption = showManualPicker && manualOption ? manualOption : selected;
  const manualExecutionBlocked = showManualPicker && manualOption?.executionStatus !== "READY";
  ...
  if (derived.activeSessionId && derived.selectedOption) {
    ...
    return (
      <section className="max-h-[calc(100dvh-8rem)] overflow-y-auto overscroll-contain rounded-2xl border border-emerald-500/30 bg-emerald-500/[0.05] p-5" data-testid="optional-study-session">
        ...
        {active.executionPacket && <div className="mt-4"><StudyExecutionPacketView packet={active.executionPacket} compact /></div>}
  ...
  return (
    <section className="rounded-2xl border border-cyan-500/25 bg-cyan-500/[0.04] p-5" data-testid="optional-study-card">
      ...
      <article className="mt-5 rounded-xl border border-zinc-800 bg-zinc-950/55 p-4"><div className="flex flex-col gap-3 sm:flex-row sm:items-start sm:justify-between"><div><p className="text-xs font-semibold text-cyan-200">Recomendação principal</p><h3 className="mt-1 text-base font-bold text-zinc-100">{METHOD_LABELS[displayedOption.method] ?? displayedOption.method}</h3><p className="mt-1 text-xs text-zinc-500">{displayedOption.disciplineName} · {displayedOption.topicName}{displayedOption.subtopicName ? ` · ${displayedOption.subtopicName}` : ""}</p></div><span className="rounded-lg border border-zinc-700 px-3 py-2 text-xs text-zinc-300"><Clock3 className="mr-1 inline h-4 w-4" />{chosenDuration} min</span></div><p className="mt-3 text-sm leading-relaxed text-zinc-300">{displayedOption.objective}</p><dl className="mt-4 grid gap-3 text-xs sm:grid-cols-2"><div><dt className="text-zinc-500">Por quê</dt><dd className="mt-1 text-zinc-300">{displayedOption.rationale}</dd></div><div><dt className="text-zinc-500">Critério de conclusão</dt><dd className="mt-1 text-zinc-300">{displayedOption.completionCriterion}</dd></div><div><dt className="text-zinc-500">Ambiente/material</dt><dd className="mt-1 text-zinc-300">{displayedOption.executionPacket?.materialTitle ?? displayedOption.materialLabel ?? displayedOption.executionPacket?.environment ?? "Ambiente ainda não validado"}</dd></div><div><dt className="text-zinc-500">Correspondência do material</dt><dd className="mt-1 text-zinc-300">{displayedOption.executionPacket?.materialMatchLabel ?? "Correspondência ainda não validada"}</dd></div></dl>{(displayedOption.warnings.length > 0 || overLimit) && <div className="mt-3 rounded-lg border border-amber-500/25 bg-amber-500/5 p-3 text-xs text-amber-200">{[...displayedOption.warnings, ...(overLimit ? ["Essa duração ultrapassa sua disponibilidade diária normal de 120 minutos. Confirme somente se hoje você possui disponibilidade excepcional."] : [])].join(" ")}</div>}</article>
      {displayedOption.executionPacket && <div className="mt-4"><StudyExecutionPacketView packet={{ ...displayedOption.executionPacket, durationMinutes: chosenDuration }} /></div>}
      ...
      {showManualPicker && <div className="mt-4 grid gap-3 rounded-xl border border-zinc-800 bg-zinc-950/45 p-4 sm:grid-cols-2" data-testid="optional-manual-picker">
        ...
        <label className="text-xs text-zinc-400">Ambiente<select value={manualEnvironment} onChange={(event) => setManualEnvironment(event.target.value as OptionalStudyRecommendationOption["environment"])} className={FIELD}>{(["notebooklm", "qconcursos", "treino_fgv", "concurseiroos", "material", "manual"] as const).map((item) => <option key={item} value={item}>{{ notebooklm: "NotebookLM", qconcursos: "QConcursos", treino_fgv: "Treino FGV", concurseiroos: "Sessão guiada", material: "Material interno", manual: "Ambiente externo" }[item]}</option>)}</select></label>
        ...
        {(manualOption?.warnings.length || manualExecutionBlocked) ? <p className="sm:col-span-2 text-xs text-amber-200">{[...(manualOption?.warnings ?? []), ...(manualExecutionBlocked ? ["Não existe caminho executável válido para esta combinação."] : [])].join(" ")}</p> : null}
      </div>}
